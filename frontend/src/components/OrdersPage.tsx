...
export const OrdersPage: React.FC<Props> = ({ defaultOrderType = "dine-in" }) => {
  ...
  const generateKOTContent = useCallback(
    (items: CartItem[], isAdditional = false, department?: string) => {
      ...
      if (formatType === "compact") {
        // Compact format
        content = `<!doctype html><html><head><meta charset="utf-8"><title>${orderNumber}</title><style>body{font-family:Arial,Helvetica,sans-serif;font-size:${fontSize};padding:${padding}} .h{text-align:center;font-weight:700}</style></head><body>` +
          `<div class="h">KOT</div>` +
  ...
  const generateBillContent = useCallback(() => {
    ...
    if (formatType === "compact") {
      // Compact bill format
      content = `<!doctype html><html><head><meta charset="utf-8"><title>${billNumber}</title><style>body{font-family:Arial,Helvetica,sans-serif;font-size:${fontSize};padding:${padding}}</style></head><body>` +
